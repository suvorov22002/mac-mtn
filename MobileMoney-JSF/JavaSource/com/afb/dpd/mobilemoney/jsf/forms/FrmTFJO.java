...
//	private String nameECRegul = "EC_Regul_Facturation_MAC_MTN-";
	...
	public String getTitle() {
		// TODO Auto-generated method stub
		return "Comptabilisation périodique des abonnements Pull/Push";
	}
	...
	public void executerTFJO(){
		...
		try {
			...
			// Msg d'information
			if(fact.isEmpty()) PortalInformationHelper.showInformationDialog("Aucune comptabilisation a effectuer cette période!", InformationDialog.DIALOG_INFORMATION);
...
//			logger.info("CHECK OK : CHARGEMENT");
			...
			logger.info("OK");
			// Msg d'information
			if(fact.isEmpty()) PortalInformationHelper.showInformationDialog("Aucune comptabilisation en REGUL à traiter, veuillez r�essayer!", InformationDialog.DIALOG_INFORMATION);
			else postFile = Boolean.TRUE;
			numPage = 1;
			num = 1;
			nbPages = (fact.size()/nbParPage) + 1;
...
//			MobileMoneyViewHelper.appManager.exportRapprochmentBkmvti(ecFileNameRapp, ecritures, ecrituresCli, ecrituresCOM, ecrituresTAX, ecrituresCliRapp, ecrituresCOMRapp, ecrituresTAXRapp, ecrituresCliUnijamb, ecrituresCOMUnijamb, ecrituresTAXUnijamb, montants);
			...
			result = result + ">>> Écritures compensation unijambitses COM : " + "\n";
			for (bkmvti e : ecrituresCOMUnijamb) {
				result = result + "   *** " + e.getLib() + "__" + e.getMon() + "\n";
			}
			
			result = result + ">>> Écritures compensation unijambitses TAX : " + "\n";
...
//			logger.info("ECRITURES FACT: "+ecritures+" || TAILLE: "+ecritures.size());
			...
			if(fac == null ){
				...
				PortalInformationHelper.showInformationDialog("Aucune ecriture de facturation post�e dans le core banking !!!", InformationDialog.DIALOG_ERROR);
				fact.clear();
				return;
	...
	//				frapp.delete();
					...
					PortalInformationHelper.showInformationDialog(trx.size()+" abonnements factur�s, "+trx_regul.size()+" en regul sur "+fact.size()+" transactions\n "+msg+" ont �t� post�es avec succ�s dans le Core Banking!", InformationDialog.DIALOG_INFORMATION);
...
//		
//		if(status==null) {
//			PortalInformationHelper.showInformationDialog("Veuillez d'abord charger les opérations à comptabiliser", InformationDialog.DIALOG_WARNING);
//			status = null;
//			return;
//		}
//
